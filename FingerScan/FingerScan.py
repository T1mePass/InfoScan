...
if __name__ == "__main__":
    ...
    parser.add_argument('-u','--url',dest="target_url",required=True, help="设置目标url")
    parser.add_argument('-p',dest="use_proxy",help="是否使用代理",default=False)
    parser.add_argument('-m',dest="m_count",help="指纹匹配的线程数，不指定时默认为50",default=50,type=int)
    parser.add_argument('-t',dest="request_timeout",help="网站响应时间，默认为5s",default=5,type=int)
    parser.add_argument('-d',dest="dir_mode",help="是否启用目录匹配式探测，0-不启用，1-启用，默认不启用",default=0,type=int)
    ...
    if use_proxy:
        ...
        if os.path.exists('proxies_ips.txt'):
            for proxy_tmp in  open('proxy_ips.txt'):
                proxy_list.append(proxy_tmp.strip())

        else:
            cprint("读取代理列表出错，确保代理文件存在，格式:124.235.123.234:8080")
        
        
    if re.match(r'https|http?://{2}\w.+$',target_url):

        print()
        print(f"current task :{target_url}")

        cms  =Cmsscanner(target=target_url)
        fofa_finger = cms.run()
